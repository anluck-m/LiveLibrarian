...
- **絞り込みの即時切り替え** — 「すべて」を表示した時点で、上位150冊ぶんの本と貸出状況をまとめて読み込んでおきます（母集団）。`貸出可 ⊆ 蔵書あり ⊆ すべて` の包含関係があるため、絞り込みの切り替えはこの母集団をその場で絞るだけで済み、**サーバへ取りに行かないので待ち時間ゼロ**です。絞り込みは表示中のページだけでなく母集団全体に効き、「もっと探す」で母集団をさらに下位まで広げられます（広げた分はどの絞り込みにも効きます）。
...
- 絞り込みの判定（蔵書あり／貸出可）はサーバでは行わず、フロントが読み込み済みの母集団に対してメモリ内で行います。サーバは「生のランキング（`/api/ranking`・`/api/ranking/pages`）」と「貸出状況（`/api/availability`）」だけを返し、どの絞り込みでも同じ母集団を使い回せるようにしています。カーリルの `check` は件数によらず所要時間がほぼ一定なので、母集団ぶんをまとめて1回で問い合わせるのが最も効率的です。
